...
const ExploreBlogs = () => {
  const { blogs, setBlogs, fetchAllBlogs } = useBlogContext();
  const [display, setDisplay] = useState("full");

  const handleDisplayChange = (e) => {
    setDisplay(e.target.value);
  };
  // may not need to have this run in context if it is running here
  useEffect(() => {
    fetchAllBlogs();
  ...
  return (
    <>
      <h1 className="text-gray-100 text-6xl text-center my-24">Explore Blogs</h1>
